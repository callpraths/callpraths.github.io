...
I will build over this simple method through the rest of the post. Signficantly, assume that compressing the
...
> I highly encourage you to plary around with the Chronotes app above, as well as later in this post. I believe that
...
- The actual compression is pulled out into `async saveInternal()` that wraps the
...
latency dashboard start showing some weird results as Perry's teammates continue to add other features to the popular
app.
...
In the first listing, upon encountering `await prepare(notes)` , `saveInternal()` returned immedaitely from line 10,
deferring the work from line 11 to 14 as a task. `save()` is able to continue synchronously and stop the timer. The
derferred task runs after `save()` has returned and incurs cost of calling `compile()`. The execution order of the
...
- Another latency measurement at a higher level of the stack broadly measured the responsiveness of the application. It
  was reporting high latency numbers (because, as we now understand there were many intervening `await` points
...
the UI. All the changes Perry and his team have made so far only move the JavaScript execution order within a single
JavaScript task. So, the browser isn't getting a chance to run the UI update task until the JavaScript execution
completes.
...
Isn't that a beauty? The note appears in the list immediately, the clock and animation run smoothly while the
compression is in progress, and the reported latency of the overall compression is accurate - more than a second because
splitting up the work does not make it go any faster.
...
that nothing needed to be updated on the screen. It is my position that that React makes it too easy for application
...
On balance, after a few years working in the infrastructure team of a heavy-weight web application, I would reach
for a less magical framework for web UI than React. I think [`Vue`] is a saner choice. Or, you can go full vanilla and
drop down to the the web platform (that's what the web was all about anyways, right?). I wrote `Chronotes` as a
...
LLMs because we were still in ~~the last century~~ 2024, and also because I wouldn't learn anything if LLM does it all.
Picking it up again this year, I added a bunch of features, like the _trace viewer_ panel. I used LLMs heavily for this
work. Feature development on an already opinionated codebase is where LLMs really shine. Finally, I can't write CSS to
save my life, so I let the LLM spin on that (and spin it did, even for tiny layout changes. LLMs are _not_ up to scratch
on getting CSS layout right yet, me thinks).
...
But the abstraction breaks when you have a chunk of CPU-heavy work occuring on the main thread. I find the
...
[chronotes-repo]: https://github.com/callpraths/callpraths.github.io/tree/52c6d85d1207d343944065ad64c0b1455d1a89f4/assets/js/article/browser-task-queues/components
